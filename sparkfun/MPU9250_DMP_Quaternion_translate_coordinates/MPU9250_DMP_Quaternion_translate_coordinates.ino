...
void loop() {
  ...
  x1 = x0;
  y1 = y0 * cos(roll_0) - z0 * sin(roll_0);
  z1 = y0 * sin(roll_0) + z0 * cos(roll_0);

  x2 = x1 * cos(pitch_0) + z1 * sin(pitch_0);
  y2 = y1;
  z2 = -x1 * sin(pitch_0) + z1 * cos(pitch_0);

  x3 = x2 * cos(yaw_0) - y2 * sin(yaw_0);
  y3 = x2 * sin(yaw_0) + y2 * cos(yaw_0);
  z3 = z2;

  float pitch_corrected = asin(z3) * 180 / PI;  //asin(z3) * 180 / PI;  //vectors_angle(1, 0, 0, x3, y3, z3);

  if (calibrated) {
    SerialPort.println("[x, y, z] = [" + String(x3) + " " + String(y3) + " " + String(z3) + "]  " + "[roll, pitch, yaw] = [" + String(roll_0) + " " + String(pitch_0) + " " + String(yaw_0) + "]" + " pitch_corrected = " + String(pitch_corrected) + " pitch = " + String(round(pitch_0 * 180 / PI)));
