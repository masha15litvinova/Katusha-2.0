    ...
    case (LINE):
      {
        ...
        err_line_sens = 0;
        for (int i = 0; i < 3; i++) {
          err_line_sens = err_line_sens + (robot.sensors[i] - robot.sensors[5 - i]) * weights[i];
        }

        robot.up_line = err_line_sens * kp_line;
        ...
        //if(!((robot.sensors[0]<grey[0])and(robot.sensors[1]<grey[1])and(robot.sensors[2]<grey[2])and(robot.sensors[3]<grey[3])and(robot.sensors[4]<grey[4])and(robot.sensors[5]<grey[5])))
        // {
        ...
        if ((robot.sensors[0] + robot.sensors[1] + robot.sensors[2] + robot.sensors[3] + robot.sensors[4] + robot.sensors[5]) > GREY_THRESHOLD * 4)  //условие перекрестка
        {
          /*motors(0, 0);
            delay(300);
    ...
    case (LINE_READ_DATA):
      {
        ...
        for (int i = 0; i < 6; i++) {
          ...
          robot.sensors[i] = map(s[i], white[i], black[i], 0, 100);  //приведение к одному диапазону, т.к. разные поверхности по-разному отражают
