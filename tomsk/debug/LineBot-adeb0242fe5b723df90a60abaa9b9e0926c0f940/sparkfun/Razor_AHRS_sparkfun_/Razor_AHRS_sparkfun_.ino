...
float ACCEL_X_MIN = -290.28;
float ACCEL_X_MAX = 261.47;
float ACCEL_Y_MIN = -257.93;
float ACCEL_Y_MAX = 265.38;
float ACCEL_Z_MIN = -349.00;
float ACCEL_Z_MAX = 271.73;

// Magnetometer (standard calibration mode)
// "magn x,y,z (min/max) = X_MIN/X_MAX  Y_MIN/Y_MAX  Z_MIN/Z_MAX"
float MAGN_X_MIN = -154.54;
float MAGN_X_MAX = 724.68;
float MAGN_Y_MIN = -1360.84;
float MAGN_Y_MAX = -429.11;
float MAGN_Z_MIN = -853.71;
float MAGN_Z_MAX = 91.52;
...
void loop() {
  ...
  if (LOG_PORT.available() >= 2) {
    if (LOG_PORT.read() == '#')  // Start of new control message
    {
      ...
      {
        if (digitalRead(STATUS_LED_PIN) == HIGH) digitalWrite(STATUS_LED_PIN, LOW);
