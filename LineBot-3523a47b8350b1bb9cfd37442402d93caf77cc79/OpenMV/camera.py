...
class greenSquares():
    ...
    global x_size
    global y_size
    green_thresholds = [0, 45, -128, -10, -128, 127] #порог зеленого цвета
    black_thresholds = [0, 35, -9, 127, -128, 127] #порог черного цвета
    min_area = 600    #минимальная площадь квадратов
    min_pixels = 600
    height = 60
    height_k = 0.8
    MAX_AREA = 3200
    ...
    def markers_array(self):
        ...
        for i in range(self.count):
            #print(self.green_markers[i].area())
            if (self.green_markers[i].area()<self.MAX_AREA):
                self.m_array.append(list(self.green_markers[i].min_corners()))
        self.count = len(self.m_array)
    ...
    def perimeter_colors(self):    #array with elements like [0, 1, 1, 0] 0-white 1-black  [left, right, down, up]
        ...
        for i in range(len(self.centres_array)):
            ...
            if l_roi[2]*l_roi[3] == 0: #checking if roi is rectangle
                ...
                if len(self.cam_image.find_blobs([self.black_thresholds],False,roi=l_roi, pixels_threshold=(length**2)//5))>0:
                    if self.DRAWING_SQR:
                ...
                if len(self.cam_image.find_blobs([self.black_thresholds],False,roi=r_roi, pixels_threshold=(length**2)//5))>0:
                    if self.DRAWING_SQR:
                ...
                if len(self.cam_image.find_blobs([self.black_thresholds],False,roi=d_roi, pixels_threshold=(length**2)//5))>0:
                    if self.DRAWING_SQR:
                ...
                if len(self.cam_image.find_blobs([self.black_thresholds],False,roi=u_roi, pixels_threshold=(length**2)//5))>0:
                    if self.DRAWING_SQR:
    ...
    def turn_direction(self):
        ...
        if(len(self.perimeter_array)==0):
            ...
            if((self.perimeter_array[0][5]*self.perimeter_array[1][5])):
                ...
                direct = 3
        elif (len(self.perimeter_array)==3): # [...,...,l, r, d, u] 0-white, 1-black
        ...
        if self.DRAWING_SQR:
            ...
            if direct == 0:
                self.cam_image.draw_string(10, 10, "Don't know [0]", color=(255,255,0), scale=2)
            elif direct == 1:
                self.cam_image.draw_string(10, 10, "Right [1]", color=(255,255,0), scale=2)
            elif direct == 2:
                self.cam_image.draw_string(10, 10, "Left [2]", color=(255,255,0), scale=2)
            elif direct == 3:
                self.cam_image.draw_string(10, 10, "Straight forward [3]", color=(255,255,0), scale=2)
            elif direct == 4:
                self.cam_image.draw_string(10, 10, "Backward [4]", color=(255,255,0), scale=2)
...
class Line():
    ...
    black_thresholds = [0, 32, -8, 127, -128, 127] #порог черного цвета
    white_thresholds = [63, 86, -128, 127, -128, 127]
    min_area = 170
    min_pixels = 170
    height_k = 0.8
    height = 0
            ...
            if self.line_fr:
                ...
                for x in self.line_fr:
                    ...
                    if len(self.line_fr)==1:
                        ...
                        length_roi = 50
                        #self.cam_image.draw_rectangle(x.cx()-length_roi//2, x.cy()-length_roi//2, length_roi, length_roi,(255,0,255), 2, False)
                        roi_white = (x.cx()-length_roi//2, x.cy()-length_roi//2, length_roi, length_roi)
                        #self.cam_image.draw_rectangle(x.cx()-length_roi//2, x.cy()-length_roi//2, length_roi, length_roi,(255,0,255), 2, False)

                        white_blobs = self.cam_image.find_blobs([self.white_thresholds], area_threshold = 200, pixels_threshold = 200, roi=roi_white)
                        #for wb in white_blobs:
                            #self.cam_image.draw_circle(wb.cx(), wb.cy(), 6, (0, 255, 0), 2, False)
                        if (d < dist_min): #and (len(white_blobs)>0):
                            s = x
                            dist_min = d
                            count_line_fr+=1
...
class Info_send():
    ...
    def __init__(self):
        ...
        red_thresholds = [0 , 0, 0, 0, 0, 0]
        #self.red_lines = self.cam_image.find_blobs([self.red_thresholds], area_threshold = 3000, pixels_threshold = 3000)
        self.arr_line_info = []
        for i in range(freq):
    ...
    def transmit_line_data(self, line_info, dir_info, angle_info, img):  #ball_info - координаты центров [белый, белый, черный]
        ...
        converted_line_info = self.map_value(avg_line_info, -7000, 7000, 0, 250)
        converted_angle_info = self.map_value(angle_info, -60, 60, 0, 250)

        self.cam_image.draw_string(300, 10, str(converted_line_info), color=(255,0,255), scale=2)
...
while(True):
    ...
    if mode:  # это режим линии
        ...
        if len(gS.angles_array) != 0:
            ...
            info.transmit_line_data(lines.sqr_avg_deviation, direction, avg_aa, img)
        else:
            info.transmit_line_data(lines.sqr_avg_deviation, direction, 0, img)
        #if len(gS.angles_array) != 0:
